...
class LiquidCrystalArabic : public LiquidCrystal_I2C {
private:
    uint8_t lcd_cols;
    uint8_t lcd_rows;
    ...
    public: LiquidCrystalArabic(uint8_t lcd_addr, uint8_t lcd_cols, uint8_t lcd_rows): LiquidCrystal_I2C(lcd_addr, lcd_cols, lcd_rows),
    lcd_cols(lcd_cols),
    lcd_rows(lcd_rows),
    cursor_position(0),
    arabic_text(new uint8_t[lcd_cols * lcd_rows]),
    arabic_text_form(new uint8_t[lcd_cols * lcd_rows]),
    arabic_text_size(0) {}

    void setCursorArabic(uint8_t x, uint8_t y) {
        LiquidCrystal_I2C::setCursor(lcd_cols - 1 - x, y);
